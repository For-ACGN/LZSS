...
//
//	chainLen=1:  fastest match    (~36%), ~139 MB/s
...
func compressWithSingleHashCandidate(data []byte, windowSize int) []byte {
	...
	for dataPtr < dataLen {
		...
		if rem >= minMatchLength {
			...
			stored := hashTable[h]
			if stored != 0 {
				cdd := resolveCandidate(stored, dataPtr)
				dist := dataPtr - cdd
				if dist > 0 && dist <= windowSize {
					...
					matchLen := 0
					for matchLen < maxLen && data[cdd+matchLen] == data[dataPtr+matchLen] {
						matchLen++
					}
...
func Decompress(data []byte) ([]byte, error) {
	...
	for dataPtr < dataLen {
		...
		if flag[flagIdx] {
			...
			mark := uint16(data[dataPtr]) | uint16(data[dataPtr+1])<<8
			offset := int(mark>>4 + 1)
